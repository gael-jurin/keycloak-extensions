...
public class MobileNumberRequiredAction implements RequiredActionProvider {
    ...
    public void requiredActionChallenge(RequiredActionContext context) {
        ...
        if (isAdherent(user)) {
            ...
            if (Strings.isNullOrEmpty(codeTiers)) {
    ...
    public void autoLoadMobileNumber(UserModel user, StringBuilder finalNumber) {
        if (isAdherent(user)) {
            String codeTiers = user.getFirstAttribute(CODE_TIERS_ATTRIBUT_KEYCLOAK);
            MesInfosService mesInfosService = new MesInfosService(apiUrl, apiKey);
            try {
                DataProfil dataProfil = mesInfosService.getDataProfil(codeTiers);
                if (buildTelFromDataReturned(finalNumber, dataProfil)) {
                    return;
                }
    ...
    private static boolean buildTelFromDataReturned(StringBuilder finalNumber, DataProfil dataProfil) {
        if (StringUtils.isNotEmpty(dataProfil.getTelPE164())) {
            ...
            formatPhoneNumber(finalNumber, telephone);
        }
        return false;
    }

    private static void formatPhoneNumber(StringBuilder finalNumber, Telephone telephone) {
        if (StringUtils.isNotEmpty(telephone.getTelNumero())) {
            // update user with mobile number  and skip initial form
            String mobileNumber = telephone.getTelNumero().trim().replace("\\.", "");
